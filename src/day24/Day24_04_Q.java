...
public class Day24_04_Q extends Frame implements ActionListener {
	...
	// 텍스트 에어리어 멤버 변수
	TextArea p_stayViewAll_pCenter_ta_viewAll; // 예약 현황보기
	...
	public void initDisplay() {
		...
		this.add("mainDisplay",new MainDisplay("숙박 예약관리 프로그램 v2.0"));
	...
	public void processStayCancel() {
		...
		try {
			...
			if (stayState.equals("빈방")) { // 빈방일 경우
				p_stayCancel_pSouth_lb_message.setText("메세지 : 해당 " + inputUserNum + " 호실은 빈방 입니다.");
				p_stayCancel_pNorth_pSouth_tfCenter_inputNumber.setText("");
				return;
			} else if (stayState.equals("예약됨")) { // 예약된 방일 경우
				hmStay.put(inputUserNum, "빈방");
				p_stayCancel_pSouth_lb_message.setText("메세지 : 해당 " + inputUserNum + " 호실이 취소 처리됨.");
				p_stayCancel_pNorth_pSouth_tfCenter_inputNumber.setText("");
				return;
			}
		} catch (NullPointerException e) {
			p_stayCancel_pSouth_lb_message.setText("메세지 : 해당 호실은 없는 호실입니다.");
